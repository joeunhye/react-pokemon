...
const PokeCard = ({ url, name }) => {
	...
	async function fetchPokeDetailDate() {
		try {
		...
		<>
			...
			{pokemon && (
				<Link to={`/pokemon/${name}`} className={`box-border rounded-lg ${border} w-[8.5rem] h-[8.5rem] z-0 bg-slate-800 justify-between items-center`}>
					<div className={`${text} h-[1.5rem] text-xs w-full pt-1 px-2 text-right rounded-t-lg`}>#{pokemon.id.toString().padStart(3, "00")}</div>
					<div className={`w-full flex items-center justify-center`}>
						<div className={`box-border relative flex w-full h-[5.5rem]`}>
							<LazyImage src={img} alt={name} />
						</div>
					</div>
					<div className={`${bg} text-center text-xs text-zinc-100 h-[1.5rem] rounded-b-lg uppercase font-medium pt-1`}>{pokemon.name}</div>
				</Link>
			)}
		</>
	);
