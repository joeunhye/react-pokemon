...
interface NextAndProviousPokemon {
...
const DetailPage = () => {
	...
	async function fetchPokemonData(id: string) {
		...
		try {
			...
			if (pokemonData) {
				...
				const nextAndPreviousPokemon: NextAndProviousPokemon = await getNextAndPreviousPokemon(id);
	...
	const filterAndFormatDescription = (flavorText: FlavorTextEntry[]): string[] => {
		const koreanDescriptions = flavorText?.filter(text => text.language.name === "ko").map(text => text.flavor_text.replace(/\r|\n|\f/g, " "));
	...
	async function getNextAndPreviousPokemon(id: number) {
		...
		const prevResponse = pokemonData.previous && (await axios.get<PokemonData>(pokemonData?.previous));
		return {
